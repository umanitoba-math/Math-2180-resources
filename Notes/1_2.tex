...
\begin{defn}[Balls]
\label{defn:ball}
Let $(X,d)$ be a metric space with $x_0\in X$ and $r>0$. We define the \textit{ball (of radius $r$ centred around $x_0$)} to be the following set
\[
B_r(x_0):= \left\{
x\in X \, : \, d(x,x_0)<r
\right\}.
...
\end{ex}
The next notion we will introduce is that of \textit{open} and \textit{closed} sets.
\begin{defn}
...
\begin{itemize}
	...
	\item The empty set $\emptyset$ and $\R$ are \textit{both open and closed}. Exercise: Why? Time permitting, we will see why these are the \textbf{only} subsets of $\R$ that are both open and closed.
	\item The previous points highlight that the notions of open and closed are \textbf{not} negations of each other. There are sets which can be exactly one of them, neither of them, or both.
\end{itemize}
\end{eg}
The notions of open and closed sets in~\Cref{defn:open_closed} might appear very abstract. We will return to more intuitive characterisations of them. For now, let us discuss other terminology which will eventually be linked to open and closed sets.
...
\begin{rem}
	...
	\begin{itemize}
		...
		\[
		A\text{ is closed }\iff\text{ every element in $A$ is a limit point of $A$.}
		\]
	\end{itemize}
\end{rem}
